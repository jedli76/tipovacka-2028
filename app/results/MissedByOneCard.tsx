...
export default function MissedByOneCard({
  tips,
  totalWithResult,
}: {
  tips: MissedByOneTip[]
        ...
        >
          <div style={{
            ...
            <div style={{ padding: '18px 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ fontWeight: 800, fontSize: '1rem', color: '#fff' }}>Uteklo o gól</span>
                <span style={{ marginLeft: 10, background: 'rgba(251,146,60,0.15)', border: '1px solid rgba(251,146,60,0.3)', borderRadius: 99, padding: '2px 10px', fontSize: '0.78rem', fontWeight: 700, color: '#fb923c' }}>
                  {tips.length} zápasů
                </span>
              </div>
              <button
            ...
            <div style={{ overflowY: 'auto', padding: '12px 16px' }}>
              {tips.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.2)', fontSize: '0.9rem' }}>
                  Zatím žádné tipy, které utekly o gól.
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {tips.map(t => (
                    <div key={t.match_id} style={{
                      background: t.is_joker ? 'linear-gradient(135deg, rgba(245,158,11,0.1) 0%, rgba(251,191,36,0.05) 100%)' : 'rgba(251,146,60,0.06)',
                      border: `1px solid ${t.is_joker ? 'rgba(245,158,11,0.35)' : 'rgba(251,146,60,0.25)'}`,
                      borderRadius: 12,
                      padding: '12px 14px',
                      display: 'flex',
                      alignItems: 'center',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.25)', fontWeight: 700, textTransform: 'uppercase' as const, marginBottom: 2 }}>Tip</div>
                          <div style={{ fontWeight: 900, color: '#fb923c', fontSize: '1rem' }}>{t.tip_home}:{t.tip_away}</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ fontWeight: 900, color: '#fff', fontSize: '1rem' }}>{t.actual_home}:{t.actual_away}</div>
                        </div>
                        <div style={{ background: t.is_joker ? 'rgba(245,158,11,0.15)' : 'rgba(251,146,60,0.12)', border: `1px solid ${t.is_joker ? 'rgba(245,158,11,0.3)' : 'rgba(251,146,60,0.25)'}`, borderRadius: 10, padding: '6px 12px', textAlign: 'center' as const, minWidth: 52 }}>
                          ...
                          <div style={{ fontWeight: 900, fontSize: '1.2rem', color: t.is_joker ? '#f59e0b' : '#fb923c' }}>{t.points}</div>
                        </div>
                      </div>
                    </div>
                  ))}
